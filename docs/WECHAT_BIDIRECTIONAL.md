...
**若 Vercel 日志出现 `bad decrypt`：** 说明解密用的密钥与企业微信当前使用的不一致。请按下面做一次「重置密钥」：

1. 企业微信管理后台 → 该自建应用 → **接收消息**。
2. 点击 **「随机获取」** 重新生成 **EncodingAESKey**，点**保存**（会提示需重新校验 URL，先不管）。
3. **立刻**把刚生成的 **43 位** EncodingAESKey **完整复制**（不要多空格、换行、引号），到 Vercel → 该项目的 **Environment Variables** → 编辑 **WECHAT_ENCODING_AES_KEY**，粘贴为新值并保存。
4. 在 Vercel 的 **Deployments** 里对该项目做一次 **Redeploy**，等部署完成。
5. 回到企业微信「接收消息」页面再点**保存**，完成 URL 校验。

这样可排除「旧密钥」「复制错位」「多环境用错密钥」等问题。
