...
按下面顺序做，可避免「URL 校验失败」或「bad decrypt」：
...
2. **我的企业** → **企业信息**，复制 **企业ID (CorpID)**，存到记事本。
3. **应用管理** → **自建** → 选中你们的自建应用（没有就「创建应用」并记下 AgentId、Secret）。
4. 进入该应用 → **接收消息** → **设置 API 接收**。
5. 填写：
   - **URL**：`https://www.jiayi.co/api/wechat/receive`（必须已部署、可访问）。
   - **Token**：自定义英文/数字，≤32 位，例如 `jiayi2026`，记下来。
   - **EncodingAESKey**：点击 **「随机获取」**，复制输入框里出现的 **43 位**密钥（只复制密钥，不要引号、空格、换行），存到记事本。
6. **先不要点「保存」**（否则会马上触发 URL 校验，此时 Vercel 还没配好会失败）。

---

### 第二步：Vercel 配置环境变量

1. 打开 [Vercel](https://vercel.com) → 进入 **jiayi** 项目 → **Settings** → **Environment Variables**。
2. 添加或编辑以下变量（**Production、Preview 都勾选**，避免预览部署用不到变量）：

   | 变量名 | 值 | 说明 |
   |--------|-----|------|
   | `WECHAT_CORP_ID` | 第一步第 2 步复制的企业ID | 无引号 |
   | `WECHAT_CALLBACK_TOKEN` | 第一步第 5 步填的 Token | 与后台完全一致 |
   | `WECHAT_ENCODING_AES_KEY` | 第一步第 5 步复制的 43 位密钥 | 无引号、无空格、无换行 |
   | `WECHAT_APP_SECRET` | 该自建应用的 Secret（应用详情页） | 可选，用于把会员回复推到顾问应用 |
   | `WECHAT_AGENT_ID` | 该自建应用的 AgentId（数字） | 可选，与上同用 |

3. 粘贴时**不要**在值前后加引号；若从别处复制，注意不要多出空格或换行。
4. 点 **Save** 保存环境变量。
...
2. 对当前项目点 **Redeploy**（或推送代码触发部署）。
3. 等待部署状态变为 **Ready**。

---

### 第四步：在企业微信后台完成 URL 校验

1. 回到企业微信管理后台 → 该自建应用 → **接收消息**（URL、Token、EncodingAESKey 仍是第一步填的）。
2. 点击 **「保存」**。
3. 若提示「保存成功」或「URL 校验通过」即成功；若失败，看下方排错。

---

### 第五步：顾问身份绑定

1. 网站 **管理后台** → **RCIC 顾问审核** → 找到要收企业微信回复的顾问 → **查看详情**。
2. 在 **「企业微信账号」** 里填写该顾问在**企业微信通讯录里的成员 UserID**（与回调里的 FromUserName 一致；若不知道，可先在企业微信应用里发一条消息，到 Vercel 日志里看 `[WeChat receive] from: "xxx"`，把 `xxx` 填到这里）。
3. 保存。
...
1. 让**会员**在网站该顾问的案件下发一条消息，且此时**顾问保持离线**（不登录后台）。
2. 企业微信群里应收到「跟进人不在线」的通知。
3. 顾问在**企业微信**里：**工作台**打开该自建应用，或**群聊里 @ 该自建应用**，发一条**文字**。
4. 会员在网站 **会员中心 → 消息** 里应能看到顾问的这条回复。

若某一步失败，见下文「排错」和「重置密钥」。
...
# 以下用于「网站 → 顾问企业微信应用」推送会员回复内容（与接收消息同一应用）
WECHAT_APP_SECRET="该自建应用的 Secret（应用详情页可见）"
WECHAT_AGENT_ID="该自建应用的 AgentId（应用详情页可见，数字）"
```

- **只做「企业微信 → 网站」**（顾问在应用里回复同步到网站）：前三项即可。  
- **还要「网站 → 企业微信应用」**（会员在网站回复推送到顾问应用、带回复内容）：再配置 `WECHAT_APP_SECRET`、`WECHAT_AGENT_ID`，顾问在**同一自建应用**里既能收到会员回复，也能直接回复，对话集中。
